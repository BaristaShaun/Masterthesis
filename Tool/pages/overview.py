...
def app():
    ...
    view = st.sidebar.radio("Select View", ["Fuel Overview Comparison"])
    ...
    # -----------------------
    # 📊 Fuel Overview Comparison
    # -----------------------
    if view == "Fuel Overview Comparison":
        ...
        default_colors = {'STL': '#FF0000', 'PTL': '#0000FF', 'PBTL': '#90EE90', 'BTL': '#008000', 'HEFA': '#FFA900'}

        st.sidebar.markdown("### 🎨 Fuel Color Settings")
        color_map = {fuel: st.sidebar.color_picker(f"{fuel} Color", default_colors[fuel]) for fuel in color_order}
        ...
        # 📈 Main chart
        fig = px.bar(
            agg_df, x='Variant', y=selected_impact, color='Fuel type',
        ...
        fig.update_yaxes(title_text="pt/kg fuel")
        st.plotly_chart(fig)
        st.dataframe(agg_df[["Variant", selected_impact, "Fuel type"]].style.format(precision=10))
